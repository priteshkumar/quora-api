...
public class QuestionControllerTest {
  ...
  //This test passes when a question is created with signed in user , having valid auth token
  @Test
  public void createQuestionWithExistingAccessToken() throws Exception {
    mvc.perform(MockMvcRequestBuilders.post("/question/create?content=my_question")
  ...
  public void getAllQuestions() throws Exception {
    mvc.perform(MockMvcRequestBuilders.get("/question/all")
  ...
  public void editQuestionWithNonExistingAccessToken() throws Exception {
    mvc.perform(
        MockMvcRequestBuilders.put("/question/edit/database_question_uuid?content=edited_question")
            .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
            .header("authorization", "non_existing_access_token"))
        .andExpect(status().isForbidden())
        .andExpect(MockMvcResultMatchers.jsonPath("code").value("ATHR-001"));
  ...
  public void editNonExistingQuestion() throws Exception {
    mvc.perform(MockMvcRequestBuilders
        .put("/question/edit/non_exisitng_question_uuid?content=edited_question")
        .contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
        .header("authorization", "database_accesstoken1"))
  ...
  public void deleteQuestionWithNonExistingAccessToken() throws Exception {
    mvc.perform(MockMvcRequestBuilders.delete("/question/delete/database_question_uuid")
        .header("authorization", "non_existing_access_token"))
        .andExpect(status().isForbidden())
        .andExpect(MockMvcResultMatchers.jsonPath("code").value("ATHR-001"));
  }
  ...
  public void deleteNoneExistingQuestion() throws Exception {
    mvc.perform(MockMvcRequestBuilders.delete("/question/delete/non_existing_question_uuid")
        .header("authorization", "database_accesstoken1"))
        .andExpect(status().isNotFound())
        .andExpect(MockMvcResultMatchers.jsonPath("code").value("QUES-001"));
  }
  ...
  public void getAllQuestionsByUserWithNonExistingAccessToken() throws Exception {
    mvc.perform(MockMvcRequestBuilders.get("/question/all/database_uuid1")
        .header("authorization", "non_existing_access_token"))
        .andExpect(status().isForbidden())
        .andExpect(MockMvcResultMatchers.jsonPath("code").value("ATHR-001"));
  }
  ...
  public void getAllQuestionsForNonExistingUser() throws Exception {
    mvc.perform(MockMvcRequestBuilders.get("/question/all/non_existing_user_uuid")
        .header("authorization", "database_accesstoken1"))
        .andExpect(status().isNotFound())
